...
// --- Reusable, Beautifully Styled Helper Components ---
...
const PredictionForm = ({ onPredictionComplete }) => { // Accept the prop
  ...
  const handleSubmit = (e) => {
    ...
    setResult(null);
    setTimeout(() => {
      const risk = Math.random() > 0.5 ? 'High Risk of Non-Survival' : 'Low Risk of Non-Survival';
      const confidence = (Math.random() * (98 - 75) + 75).toFixed(1);
      
      const newResult = { risk, confidence };
      setResult(newResult);
      setIsLoading(false);

      // *** This is the CRUCIAL part that makes the dashboard work ***
      // Check if the function exists before calling it
      if (onPredictionComplete) {
        onPredictionComplete({
          id: `P${Date.now().toString().slice(-6)}`, // Create a unique ID for the history entry
          inputs: formData,
          output: newResult,
        });
      }
